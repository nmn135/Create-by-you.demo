...
| 背景音乐 + 音效 | ✅ WebAudio 程序化 BGM + SFX + M 静音 |
| 引导移动 walk | ✅ 市长/说书人带路（walkToBell，玩家跟随 + 相机跟随） |
| 刻痕三「作者之墨归位」 | ✅ 当铺买话 → 说书人归位 → 钟楼演出 |
| 四层剧场（meta 终局） | ✅ 感官/叙事/世界/诚实（作者坦白） |
| 无限结局 | ✅ 客户端定世界线 + LLM 现写 epilogue + 隐藏结局 |
| 文字原型 text-prototype | ✅ 保留（失言系统/状态机逻辑参考） |
| 3D 方案 | ❌ 已全部删除（2026-08-13） |

## 本次会话新增（2026-08-13 深夜 → 清晨）
...
- **开场背景故事**：进入游戏先读四行开场（打字机效果），ESC/点击跳过。
- **NPC 像素肖像**：对话头像 48×48 像素肖像（市长/当铺/说书人/meta 各有程序化形象），美术出 `portrait_<id>.png` 即自动替换。
- **BGM + 音效**：WebAudio 程序化背景音乐（day7/day2 两套），对话/气泡/脚步/秘密/结局音效；M 静音。美术出 `audio/bgm_*.ogg` 即文件优先。
- **引导移动**：市长"跟我去钟楼"、说书人"归还墨带路"真的移动 + 玩家跟随 + 相机跟随。
- **刻痕三 + 结局**：完整收尾主线——作者之墨归位 → meta 四层剧场 → 玩家说最后一句话 → 世界线结局（续写/合卷/抹去/坦白之后）。LLM 现写 epilogue，离线有本地兜底。
...
- **`_harness.js`（2D demo 回归）**：**39/39**（DOM/Canvas 桩：场景渲染/存档/话题栏/相机/气泡/开场/肖像/音频/引导移动/刻痕三/四层剧场/结局/重置）
