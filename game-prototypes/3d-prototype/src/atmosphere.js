...
export class Atmosphere {
  ...
  /** 尘埃粒子：烛光下漂浮的微粒，填满殿内体积（哥特氛围） */
  _buildDustMotes() {
    const T = this.THREE;
    const w = this.HALL.width, d = this.HALL.depth, h = this.HALL.height;
    const count = 220;
    ...
    for (let i = 0; i < count; i++) {
      base[i*3]   = (Math.random() - 0.5) * w * 0.8;
      base[i*3+1] = 0.4 + Math.random() * (h - 1.2);
      base[i*3+2] = (Math.random() - 0.5) * d * 0.8;
    ...
    // 供逐帧更新使用
    this._dust = { base, speeds, seeds, bounds: { w, d, h } };
  }

  /** 每帧调用：尘埃缓慢上浮 + 水平正弦漂移 + 顶部回绕 */
  update(dt) {
    if (!this.dust) return;
    this._dustT += dt;
    const t = this._dustT;
    const attr = this.dust.geometry.attributes.position;
    const arr = attr.array;
    const { base, speeds, seeds, bounds } = this._dust;
    const n = arr.length / 3;
    for (let i = 0; i < n; i++) {
      let y = arr[i*3+1] + speeds[i] * dt;
      if (y > bounds.h - 0.5) {
        y = 0.4;
        base[i*3]   = (Math.random() - 0.5) * bounds.w * 0.8;
        base[i*3+2] = (Math.random() - 0.5) * bounds.d * 0.8;
      }
      arr[i*3+1] = y;
      arr[i*3]   = base[i*3]   + Math.sin(t * 0.35 + seeds[i]) * 0.16;
      arr[i*3+2] = base[i*3+2] + Math.cos(t * 0.28 + seeds[i] * 1.7) * 0.16;
    }
