...
- [x] 整套移植 web 版（世界状态 / 隔墙有耳 / 流言 / 刻痕 / 结局 / 存档）——完成 ✅（任务 #24~#31）
- [x] 修复跨 NPC 对话历史泄漏（"市长说神官的台词"）+ 存档防御 + 回归测试 ——完成 ✅

## 两个真实 bug 是怎么修好的（2026-08 维护记录）

**1. "市长说神官的台词"（自由输入 AI 回复串台）**

根因：`dialogue_history` 原来是一个全局桶，所有 NPC 的对话混着存。跟神官聊完再找市长，神官的回复会以 assistant 身份被塞进市长的 LLM 上下文——LLM 看到上一句"assistant"是神官，就顺着神官的口吻回。

修复（三层）：
- `game_state.gd`：每条历史带 `npc` 标记（`log_dialogue(role, text, npc)`）
- `llm_mapper.gd`：`build_talk_body` 只取**当前 NPC** 的历史；meta 频道（作者）保留全部——"你一路说过的话，这座城一个字都没忘"
- `server.js`：`/api/talk` 再做一道同人过滤（meta 全放行；老客户端无标记放行）
