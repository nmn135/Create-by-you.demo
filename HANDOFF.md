...
## 十七、2026-08-13 夜间工作流（2D demo 换引擎修糊 + 3D 氛围 + AI 管线 + 资产索引）

> 新会话 5 分钟接手版：本轮核心是**2D demo《第七天》的清晰度与响应速度**（用户实测反馈），连带 3D 场景增强、AI 管线模型名修复、资产索引扩充、失言系统测试全绿。

### 当前运行状态

| 服务 | 端口 | 状态 |
|:---:|:---:|:---:|
| 2D 叙事 demo（第七天） | 8890 | ✅ 运行中，引擎 DEEPSEEK（启动日志 `对话引擎: DEEPSEEK`） |
| 3D 原型 | 8080 | ✅ 运行中（SMOKE OK，22k 三角形） |

### 本轮关键改动

**1. 2D demo 模糊修复（用户实测反馈「太糊看不清」）**
- 根因：320×180 内部分辨率拉伸全屏 → 字糊
- 修复：`index.html` 加 `RENDER_SCALE=3` → canvas 960×540 + `setTransform(3,0,0,3,0,0)`；字体 6→8px / 8→10px，气泡与提示框加大
- 验证：`D:/tools/playwright/2d_blur_check.js` 确认 canvas 960×540

**2. 对话引擎 doubao → DeepSeek（用户问「换 key 能变快吗」）**
- 实测：DeepSeek raw **251ms**，doubao 负载高时 15-24s → 答案：换模型比换 key 有效
- `server.js` 双 Provider 解析：优先 `deepseek-chat`，doubao 兜底（好友 fork 机器无 DeepSeek key 自动用 doubao）。Key 来源：`.env` / 环境变量 / snapshot key 文件
- **安全**：doubao key 只从 snapshot 文件 `C:/Users/11988/.claude/file-history/ecca7d29-e2e2-4524-903f-7e5846e7a39e/4667fd2cf3e22d17@v3` 读取，**绝不回显/入库**

**3. 3D 场景增强（Task #49）**
- `src/atmosphere.js`：220 粒烛光尘埃粒子（AdditiveBlending、上浮+漂移+回绕）
- `index.html`：几何人形 NPC 待机呼吸（scale 正弦微幅，按名字哈希错相）；`window.__atmosphere` 暴露
- `src/models.js`：FBX 探测超时 5s→15s（大模型慢环境防误判）

**4. AI 管线模型名修复（Task #51，关键 bug）**
- `src/config.py`：`deepseek-v4-pro` / `deepseek-v4-flash` 是**无效模型名**（导致空响应/卡顿触发兜底链）→ 统一 `deepseek-chat`
- `src/ai_pipeline.py`：Prompt 从 `prompts/*.txt` 加载（intent_parser_v2.txt / reply_generator_v2.txt），启用失言 A/B/C、在场感知、对话记忆、情绪动态温度；所有 API 调用 `timeout=8`

**5. 资产索引（Task #50）**
- `ASSET_LINKS.md` 148→250 行：Mixamo 快捷搜索 / Sketchfab 哥特系 19 条 / Poly Haven HDRI+PBR / Quaternius·Kenney·OpenGameArt CC0 包

**6. 失言系统测试全绿（Task #52）**
- state_machine **30/30** · endings **46/46** · dialogue **3 策略/9 轮/0 失言**（DeepSeek 真实对话）

### 已提交并推送
- `0eebda4` — 2D demo DeepSeek + 3× 分辨率修复模糊
- `e3ce4d6` — 3D 尘埃粒子 + NPC 呼吸
- `3d0f862` — AI 管线模型名修复 + Prompt 模板化

### 待办（下一步）
- 提交本轮文档/资产改动（README、索引、NIGHTLY_REPORT 21 节、HANDOFF、ASSET_LINKS.md）
- 角色模型更换仍按 `docs/mixamo-model-guide.md` §八自动化管线；Liana Biped 待上传 Mixamo（task #10）
- 好友 fork 侧：深色方案已在好友 fork（Dark Forest 分支），主仓库为深色暖光版，合并时注意
