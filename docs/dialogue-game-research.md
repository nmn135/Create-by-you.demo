...
> 调研范围：① 除打字外的互动按键设计；② 多 NPC 对话设计；③ 对话驱动世界改变（关系/信任/秘密/后果）；④ GitHub 可借鉴的开源项目。目标：为封印之殿（Node 服务 + Python 状态机 + DeepSeek AI 的中文对话驱动叙事 demo）整理可直接落地的构思。
>
> 项目现状对照：`AGENTS.md` 已确认现有 2D 叙事 demo（`2d-narrative-demo`）具备多 NPC 站点巡游 / 隔墙有耳 / 闲聊气泡 / 实时对话；待办含「节点分支对话（骑砍式话题树）」。本文在现有 `docs/dialogue-interaction-design.md` 基础上扩展。
...
| 近身距离理论（proxemics） | 社交距离=不知密谈；个人距离=看到两人在密谈但听不到；亲密距离=听得见内容 | 用**距离**做可见性分级，天然适配带空间/位置的场景 |
...
按"成本/收益"粗略排序，均对齐现有架构（Python 状态机裁决 + AI 生成 + Canvas 2D 前端）：
...
> 落地优先级建议：1（检视层）→ 7（隐喻）→ 3（悄悄话距离）→ 2（话题树）→ 4（压裂补强）→ 5（双轴）→ 6（记忆检索）→ 8（反悔机制）。1/2/7 接近纯前端，可与现有 2D demo 直接迭代；3/4/5 主要落在 Python 状态机层；6 落在 AI 管线 prompt 工程。
